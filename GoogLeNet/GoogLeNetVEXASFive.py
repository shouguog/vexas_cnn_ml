...
for epoch in range(1, epoch+1):
    ...
    if epoch % 10 == 0:
        torch.save(model, './models/googlenet_Five_epoch_' + str(epoch) + '-vexas.pth')
...
print(model)
torch.save(model, './models/googlenet-vexas-Five.pth') #保存模型
...
if __name__ == '__main__':
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = torch.load('./models/googlenet-vexas-Five.pth')  # 加载模型
