...
print('GoogLeNet_InceptionV3VEXASFive')
train_data = torch.load('../datasetFive/train_data_299.pt')
test_data = torch.load('../datasetFive/test_data_299.pt')
...
for epoch in range(1, epoch+1):
    ...
    if epoch % 10 == 0:
        torch.save(model, './models/googlenetv3_Five_epoch_' + str(epoch) + '-vexas.pth')
...
plt.title('Accuracy')
plt.show()
plt.savefig('VEXAS_train_first4_InceptionV3')
plt.close()
print(model)
torch.save(model, './models/googlenetv3-vexas-Five.pth') #保存模型
...
if __name__ == '__main__':
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = torch.load('./models/googlenetv3-vexas-Five.pth')  # 加载模型
